...
- **THEN** it SHALL connect to `OPENAI_BASE_URL` (default: `http://localhost:8080/v1`) using the `openai` npm client with `baseURL` set to that value, and use `LOCAL_LLM_MODEL` (default: `gemma-4-12b-it-Q8_0`) as the model
...
### Requirement: Gemma 4 thinking-mode compatible JSON parsing
...
The system SHALL set `max_tokens: 4096` per section request when using `--local` mode.

#### Scenario: Token budget for thinking models
- **WHEN** translating with `--local` mode
- **THEN** each section translation request SHALL use `max_tokens: 4096` to accommodate the model's reasoning tokens (which consume ~90% of the budget)
...
- **THEN** it SHALL include all technology names, brand names, and proper nouns present in the CV data (e.g., Kubernetes, Kafka, ArgoCD, Helm Chart, MCP, FASTMCP, Akamai, Kong, GitHub Copilot CLI, OpenSpec, SDD, Jira, Confluence, Context7, PostgreSQL, AWS, EKS, Lambda, S3, Aurora, Jenkins, GitLab CI, Nexus, IBM UrbanCode Deploy, Apache Struts, Tomcat, IBM DB2, JasperReports, Spring Framework, Oracle DB, Tauri, React, Rust, SQLite, Ollama, ChromaDB, Qwen2.5, Vaadin, Angular, Hibernate)

#### Scenario: Few-shot example per section type
- **WHEN** translating a section
- **THEN** the `[EXAMPLE]` block SHALL contain a short, representative input→output pair for that section type (e.g., for `languages`: `[{"language": "Italiano", "fluency": "Madrelingua"}]` → `[{"language": "Italian", "fluency": "Native"}]`)
...
- **THEN** the script SHALL default to `gemma-4-12b-it-Q8_0`
